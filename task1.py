...
class Parameters:
    def __init__(self):
        ...
        self.dir_test_examples = os.path.join(self.base_dir,'testare') # this should be changed for other folders with testing images
        
        self.use_small_batch = False 
        self.path_annotations = os.path.join(self.base_dir, 'validare/task1_gt_validare.txt')
        
        self.path_annotations_dad = os.path.join(self.base_dir, 'validare/task2_dad_gt_validare.txt')
        self.path_annotations_mom = os.path.join(self.base_dir, 'validare/task2_mom_gt_validare.txt')
        self.path_annotations_dexter = os.path.join(self.base_dir, 'validare/task2_dexter_gt_validare.txt')
        self.path_annotations_deedee = os.path.join(self.base_dir, 'validare/task2_deedee_gt_validare.txt')
        ...
        # self.windows = ['w_0.5', 'w_0.75', 'w_1', 'w_1.25', 'w_1.5', 'w_1.75', 'w_2']
        # self.window_aspect_ratio = {'w_0.5': [2.0, 1.0], 'w_0.75': [1.5, 1.0], 'w_1': [1.0, 1.0], 'w_1.25': [1.0, 1.25], 'w_1.5': [1.0, 1.5], 'w_1.75': [1.0, 1.75], 'w_2': [1.0, 2.0]}

        self.windows = ['w_0.75_90', 'w_1_90', 'w_1.25_90', 'w_1.5_90']
        self.window_aspect_ratio = {'w_0.75_90': [4/3, 1.0], 'w_1_90': [1.0, 1.0], 'w_1.25_90': [1.0, 1.25], 'w_1.5_90': [1.0, 1.5]}
        self.window_threshold = {'w_0.75_90': 0, 'w_1_90': 0, 'w_1.25_90': 0, 'w_1.5_90': 0}
...
def show_detections_without_ground_truth(detections, scores, file_names, params: Parameters):
    ...
    for test_file in tqdm(test_files):
        ...
        detections_folder = os.path.join(params.dir_save_files, f"detections_{params.dim_window}_{params.dim_hog_cell}_{params.use_hard_mining}")
        if not os.path.exists(detections_folder):
            os.makedirs(detections_folder)
        cv.imwrite(os.path.join(detections_folder, "detections_" + short_file_name), image)
...
def show_detections_with_ground_truth(detections, scores, file_names, params: Parameters):
    ...
    for test_file in tqdm(test_files):
        ...
        if not os.path.exists(detections_folder):
            os.makedirs(detections_folder)
        cv.imwrite(os.path.join(detections_folder, f"detections_" + short_file_name), image)
        #print('Apasa orice tasta pentru a continua...')
        #cv.imshow('image', np.uint8(image))
        #cv.waitKey(0)
...
class FacialDetector:
    ...
    def get_positive_descriptors_per_window(self, window):
        ...
        character_faces = self.params.characters_faces['face'] # for 'face', we have all the faces
        for face in tqdm(character_faces):
            ...
            bigger_img = cv.resize(img, None, fx=1, fy=1, interpolation=cv.INTER_LINEAR)
    ...
    def get_negative_descriptors_for_face_per_window(self, window):
        files = []
        ...
        for i in tqdm(range(num_images)):
            ...
            for j in range(num_negative_per_image):
                ...
                patch_y_size = y_size
                for _ in range(30):
                    possible_rand_window_size = np.random.randint(low=0.75 * self.params.dim_window, high=1.25 * self.params.dim_window)
                    possible_patch_x_size = int(self.params.window_aspect_ratio[window][1] * possible_rand_window_size)
                    possible_patch_y_size = int(self.params.window_aspect_ratio[window][0] * possible_rand_window_size)
                    if possible_patch_x_size <= num_cols and possible_patch_y_size <= num_rows:
                        patch_x_size = possible_patch_x_size
                        patch_y_size = possible_patch_y_size
                        break

                if not found:
                    for _ in range(30):
                        x_rand = np.random.randint(low=0, high=num_cols - patch_x_size)
                        y_rand = np.random.randint(low=0, high=num_rows - patch_y_size)
                        patch_coordinates = np.array([x_rand, y_rand, x_rand + patch_x_size, y_rand + patch_y_size])
                        intersects = False
                        ...
                        if not intersects:
                            x.append([x_rand, patch_x_size])
                            y.append([y_rand, patch_y_size])
                            found = True
                            break
            ...
            for idx in range(len(y)):
                patch = img[y[idx][0]: y[idx][0] + y[idx][1], x[idx][0]: x[idx][0] + x[idx][1]]
                resized_patch = cv.resize(patch, (x_size, y_size))
                descr = hog(resized_patch, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell), cells_per_block=(2, 2), feature_vector=False, orientations=9)
    ...
    def train_classifier(self, training_examples, train_labels, character):
        ...
        svm_file_name = os.path.join(self.params.dir_save_files, f'best_model_rbf_pyramid_1_1.3_neg_random_sizes_orientations_9_set_{character}_{hard_mining_used}_{self.params.dim_window}_{self.params.dim_hog_cell}_{self.params.number_negative_examples}_{self.params.number_positive_examples}.pkl')
        ...
        # plt.savefig(os.path.join(self.params.dir_save_files, f'distributie_{character}.png'))
        # plt.show()
    ...
    def predict_face(self, img, initial_resize_factor, window):
        ...
        for resized_image in pyramid_gaussian(img, downscale=1.05, preserve_range=True):
            ...
            hog_descriptors = hog(resized_image, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell), cells_per_block=(2, 2), feature_vector=False, orientations=9)
            ...
            resized_image_detections = np.array(resized_image_detections)
            resized_image_scores = np.array(resized_image_scores)
    ...
    def run(self, small_batch=False):
        ...
        for i in tqdm(range(num_test_images)):
            ...
            try:
                ...
                print('Exceptie pentru imaginea %s' % test_files[i])
                print(e)
                continue
                    
            # need to apply non-maximal suppression for all windows detections
            if len(windows_scores) > 0:
                windows_detections, windows_scores, windows_file_names = self.non_maximal_suppression(np.array(windows_detections), np.array(windows_scores), img.shape, image_file_names=np.array(windows_file_names))
        ...
        return detections, scores, file_names, detections_per_window, scores_per_window, file_names_per_window
    ...
    def eval_detections(self, detections, scores, file_names):
        ...
        print(self.params.path_annotations)
        ...
        plt.title(f'Average precision = {average_precision}')
        plt.savefig(os.path.join(self.params.dir_save_files, f'precizie_medie.png'))
        plt.show()
    ...
    def save_task2(self, character_detections, character_scores, character_file_names, character_predictions=None):
        for character in self.params.characters:
            if character == 'face':
                continue
            np.save(os.path.join(self.params.task2_save_dir, f'detections_{character}.npy'), character_detections[character], allow_pickle=True, fix_imports=True)
            np.save(os.path.join(self.params.task2_save_dir, f'scores_{character}.npy'), character_scores[character], allow_pickle=True, fix_imports=True)
            np.save(os.path.join(self.params.task2_save_dir, f'file_names_{character}.npy'), character_file_names[character], allow_pickle=True, fix_imports=True)

            if character_predictions is not None:
                # save also txt with character predictions
                with open(os.path.join(self.params.dir_save_files, f'task2_predictions_{character}.txt'), 'w') as f:
...
facial_detector.params.window_threshold = {'w_0.75_90': 0, 'w_1_90': 0, 'w_1.25_90': 0, 'w_1.5_90': 0}

if facial_detector.params.use_small_batch:
    facial_detector.params.path_annotations = os.path.join(facial_detector.params.base_dir, 'validare/task1_gt_validare20.txt')
else:
    facial_detector.params.path_annotations = os.path.join(facial_detector.params.base_dir, 'validare/task1_gt_validare.txt')
...
for window in facial_detector.params.windows:
    positive_features_window_path = os.path.join(params.dir_save_files, 'descriptoriExemplePozitive_pyramid_1_1.3_orientations_9_' + str(window) + '_' + str(params.dim_window) + '_' + str(params.dim_hog_cell) + '_' + str(params.number_positive_examples) + '.npy')
    ...
    negative_features_window_path = os.path.join(params.dir_save_files, 'descriptoriExempleNegative_orientations_9_random_sizes_' + str(window) + '_' + str(params.dim_window) + '_' + str(params.dim_hog_cell) + '_' + str(params.number_negative_examples) + '.npy')
    if os.path.exists(negative_features_window_path):
        ...
        negative_features = facial_detector.get_negative_descriptors_for_face_per_window(window)
...
detections['all'], scores['all'], file_names['all'], detections_per_window, scores_per_window, file_names_per_window = facial_detector.run(small_batch=facial_detector.params.use_small_batch)



for window in facial_detector.params.windows:
    detections[window] = detections_per_window[window]
    scores[window] = scores_per_window[window]
    file_names[window] = file_names_per_window[window]
    
facial_detector.save_task1(detections['all'], scores['all'], file_names['all'])


# task 1 - evaluate detections
if facial_detector.params.has_annotations:
    
    facial_detector.eval_detections(detections['all'], scores['all'], file_names['all'])
    show_detections_with_ground_truth(detections['all'], scores['all'], file_names['all'], params)
        
else:
    show_detections_without_ground_truth(detections['all'], scores['all'], file_names['all'], params)